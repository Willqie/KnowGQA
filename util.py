...
class SQUADwithGraph(data.Dataset):
    def __init__(self,data_path, use_v2=True):
        super(SQUADwithGraph, self).__init__()
        dataset = np.load(data_path,allow_pickle=True)
        self.context_idxs = torch.from_numpy(dataset['context_idxs']).long()
        self.context_char_idxs = torch.from_numpy(dataset['context_char_idxs']).long()
        self.question_idxs = torch.from_numpy(dataset['ques_idxs']).long()
        self.question_char_idxs = torch.from_numpy(dataset['ques_char_idxs']).long()
        self.concept_idxs=torch.from_numpy(dataset["concept_idxs"]).long()
        self.y1s = torch.from_numpy(dataset['y1s']).long()
        self.y2s = torch.from_numpy(dataset['y2s']).long()
        self.edges_lists=dataset["edges_lists"]
    ...
    def __getitem__(self, idx):
        idx = self.valid_idxs[idx]
        concept_size=self.concept_idxs[idx].size(-1)
        adjaency_matrix=torch.eye(concept_size,dtype=torch.float)
        edge_list=torch.tensor(self.edges_lists[idx])
        edge_num=edge_list.size(0)
        edge_list=tuple(edge_list[:,0:2].transpose(-1,-2).type(torch.LongTensor))
        adjaency_matrix=adjaency_matrix.index_put(edge_list,torch.FloatTensor([1 for _ in range(edge_num)]))
        degree=adjaency_matrix>0
        degree=degree.sum(-1).float()
        degree_matrix=torch.diag(torch.pow(degree,-1))
        adjaency_matrix=degree_matrix.mm(adjaency_matrix)
        example = (self.context_idxs[idx],
                   self.context_char_idxs[idx],
                   self.question_idxs[idx],
                   self.question_char_idxs[idx],
                   self.concept_idxs[idx],
                   adjaency_matrix,
                   self.y1s[idx],
                   self.y2s[idx],
                   self.ids[idx])
...
def collate_fn_graph(examples):
    ...
    def clip_adjaency(matrixs,size,dtype=torch.float):
        clip_adjaency_matrix = torch.zeros(len(matrixs),size,size,dtype=dtype)
        for i,matrix in enumerate(matrixs):
            clip_adjaency_matrix[i]=matrix[:size,:size]
        return clip_adjaency_matrix


    # Group by tensor type
    context_idxs, context_char_idxs, \
        question_idxs, question_char_idxs, \
        concept_idxs, adjaency_matrixs,\
        y1s, y2s, ids = zip(*examples)
    ...
    question_idxs = merge_1d(question_idxs)
    question_char_idxs = merge_2d(question_char_idxs)
    concept_idxs=merge_1d(concept_idxs)
    concept_size=concept_idxs.size(-1)
    adjaency_matrixs=clip_adjaency(adjaency_matrixs,concept_size)
    y1s = merge_0d(y1s)
    y2s = merge_0d(y2s)
    ids = merge_0d(ids)
...
def masked_softmax(logits: object, mask: object, dim: object = -1, log_softmax: object = False) -> object:
